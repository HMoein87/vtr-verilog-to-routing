...
def run_synthesis(verilog_file, queue):
    ...
    try:
        for i in range(10):
            arch = "/mnt/comparison_results/symbiflow-arch-defs/build/xc/xc7/archs/artix7_200t/devices/xc7a200t-virt/arch.timing.xml"
            odin_command = "./scripts/run_vtr_flow.pl {} {} -delete_intermediate_files -ending_stage prevpr -abc_exe /mnt/comparison_results/symbiflow-arch-defs/env/conda/envs/symbiflow_arch_def_base/bin/yosys-abc -latch_map_script ./scripts/latch_map.py".format(
                verilog_file, arch)
            odin_def_abc_command = "./scripts/run_vtr_flow.pl {} {} -delete_intermediate_files -ending_stage prevpr -latch_map_script ./scripts/latch_map.py".format(
                verilog_file, arch)
            ...
            output_prefix = sys.argv[3]
            odin_temp = output_prefix + "/temp_" + verilog_file_name + "_odin_" + str(i)
            odin_def_abc_temp = output_prefix + "/temp_" + verilog_file_name + "_odin_abc_" + str(i)
            yosys_temp = output_prefix + "/temp_" + verilog_file_name + "_yosys_" + str(i)

            odin_command += " -temp_dir " + odin_temp
            odin_def_abc_command += " -temp_dir " + odin_def_abc_temp
            ...
            out = subprocess.run(odin_def_abc_command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
            if out.returncode != 0:
                msgs.append("Error running Odin II with Default ABC:\n" + out.stdout.decode("utf-8"))
                error |= 4
...
def run_vpr(temp_dir, i):
    ...
    try:
        vpr_args = "--device xc7a200t-test --read_rr_graph /mnt/comparison_results/symbiflow-arch-defs/build/xc/xc7/archs/artix7_200t/devices/rr_graph_xc7a200t_test.rr_graph.real.bin \
                    --max_router_iterations 500 --routing_failure_predictor off --router_high_fanout_threshold -1 --constant_net_method route --route_chan_width 500 \
                     ...
                     --read_router_lookahead /mnt/comparison_results/symbiflow-arch-defs/build/xc/xc7/archs/artix7_200t/devices/rr_graph_xc7a200t_test.lookahead.bin \
                     --read_placement_delay_lookup /mnt/comparison_results/symbiflow-arch-defs/build/xc/xc7/archs/artix7_200t/devices/rr_graph_xc7a200t_test.place_delay.bin"
        arch = "/mnt/comparison_results/symbiflow-arch-defs/build/xc/xc7/archs/artix7_200t/devices/xc7a200t-virt/arch.timing.xml"
