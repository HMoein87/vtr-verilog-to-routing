...
begin
    for(i=24;i>0;i=i-1)begin
        r_storage[i] = (reset) ? 8'b00000000 : (load_c) ? r_storage[i] : r_storage[i-1];
        coef_storage[i] = (reset) ? 8'b00000000 : (load_c) ? coef_storage[i-1] : coef_storage[i];
    end
    r_storage[0] = (reset) ? 8'b00000000 : (load_c) ? r_storage[0] : data_in;
    coef_storage[0] = (reset) ? 8'b00000000 : (load_c) ? coef_in : coef_storage[i];

    temp=0;
    for(i=0;i<25;i=i+1)begin
        temp=temp+product_array[i];
    end
    data_out=(reset | load_c) ? data_out : temp;
end
endmodule
