...
def run_vpr(temp_dir, i):
    ...
    try:
        vpr_args = "--device auto --read_rr_graph /mnt/comparison_results/symbiflow-arch-defs/build/xc/xc7/archs/artix7_200t/devices/rr_graph_xc7a200t_test.rr_graph.real.bin \
...
def run_benchmarks(verilog_files):
    ...
    with multiprocessing.Pool(threads) as thread_pool:
        ...
        for verilog_file in verilog_files:
            line = "Running VPR for " + verilog_file
            print(line, flush=True)
            print("=" * len(line), "\n", flush=True)
            verilog_file_name = verilog_file.split("/")[-1]
            procs, errs = launch_vpr(verilog_file_name, thread_pool)
